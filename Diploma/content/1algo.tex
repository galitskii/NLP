...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		...
		\ElsIf{<<Y>>$\in $ r}
		\State left $\gets$ id1$-6$ \Comment{Инициализировали границу левого операнда союза}
		\State \textbf{break}
		\EndIf
		\EndIf
		\EndFor
...
В алгоритмах \ref{alg4}, \ref{alg5} и \ref{alg6} неоднократно фигурирует функция check(). В данном случае она используется для проверки предложения, не содержащего знаки пунктуации. Её описание будет в следующем параграфе (алгоритм \ref{alg36}).
...
\begin{algorithm}[!h]
	\caption{-- Продолжение алгоритма \ref{alg8}}\label{alg32}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
...
Итак, в результате работы фрагментов \ref{alg7}, \ref{alg8} и \ref{alg32} будет сдвинута граница до <<первой>> запятой.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State end$\gets (-1)$
		\For{\textbf{from} $i = 0$ \textbf{to} len(res0)$ - 1$}\label{st1}
		\If{res0$[i][0]=$<<5>>}
...
\begin{algorithm}
	\caption{-- Продолжение алгоритма \ref{alg12}}\label{alg34}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\EndIf
...
Здесь следует рассмотреть решение проблемы омографии. Оно представлено в строках \algref{alg12}{st1}~--~\algref{alg34}{st2}. Для начала проверяем, является ли первое слово рассматриваемого предложения личным глаголом, число которого определено неоднозначно. Это имеет значение, поскольку в рамках поставленных ограничений если в предложении есть личные глаголы в повелительном наклонении, то с одного из них оно начинается. Например: \textit{<<Учите математику, высыпайтесь и будьте людьми>>}.
...
\begin{algorithm}[!h]
	\caption{-- Продолжение алгоритма \ref{alg34}}\label{alg13}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\If{id1$-6\ge 0$ \textbf{and} left$=(-1)$ \textbf{and} <<,>>$\notin$ l$[$id1$ - 6:\,$ id1$]$}
		\State sng1 $\gets [\, ]$ 
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		...
		\For{\textbf{from} $i=0$ \textbf{to} len(resl4)$ - 1$}
		\If{resl4$[i][0]=$part}
		\State sng1$\gets$ sng1 $+$ list(resl4$[i][1]$)
		\State r$\gets$ r $+$ list(check(l$[$id1$-4\, :\, $ id1 $]$))
		\EndIf
		\EndFor
		\If{len(sng1)$ > 0$}
		\If{(len(sng1)$=1$ \textbf{and} sng $\in$ sng1 \textbf{and} <<Y>>$\in$ r) \textbf{or} (len(sng1)$>1$ \textbf{and} (sng$=$<<N>> \textbf{or} pov$=1$) \textbf{and} <<Y>> $\in$ r)}
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		...
		\If{id1$-1\ge 0$ \textbf{and} left$=(-1)$}
		\State sng1 $\gets [\, ]$ 
		\For{\textbf{from} $i = 0$ \textbf{to} len(resl1)}
		\If{resl1$[i][0]=$part}
		\State sng1$\gets$ sng1 + list(resl1$[i][1]$)
		\EndIf
		\EndFor
		\State sng1 $\gets$ list(set(sng1))
		\If{(len(sng1)$=1$ \textbf{and} sng $\in$ sng1) \textbf{or} (len(sng1)$ > 1$)}
		\State left$\gets$ id1$-1$
			\Else
		\State \Return $[$<<он>>, <<писали>> $]$
			\EndIf
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\While{<<,>>$\in$ l$[1:$ left $]$}
		\If{left$-3\ge 0$ \textbf{and} l$[$left$-3]=$ <<,>> \textbf{and} l$[$left$-1]=$ <<,>>}
		\State sng1$\gets [\, ]$
		\State res1$\gets$ l$[$left$-2]$.$[$pos, singular, cow$]$
		\For{\textbf{from }$i=0$ \textbf{to} len(res1)$ - 1$}
		\If{res1$[i][0]=$part}
		...
		\EndFor
		\State sng1$\gets$ list(set(sng1))
		\If{(len(sng1 $=1$) \textbf{and} sng$\in$ sng1) \textbf{or} (len(sng1)$>1$ \textbf{and} (sng$=$<<N>> \textbf{or} pov~$=~1$))}
			\State left $\gets$ left$-2$
			\Else
			\State \Return $[$<<он>>, <<писали>> $]$
	\algstore{bkbreak}
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\EndIf
		\ElsIf{left$-4\ge 0$ \textbf{and} l$[$left$-4]=$ <<,>> \textbf{and} l$[$left$-1]=$ <<,>>}
		\State sng1$\gets [\, ]$
		\State r$\gets [\, ]$
		\State res1$\gets$ l$[$left$-3]$.$[$pos, singular, cow$]$
		\For{\textbf{from }$i=0$ \textbf{to} len(res1)$ - 1$}
		...
		\State \Return $[$<<он>>, <<писали>> $]$
		\EndIf
		\ElsIf{left$-5\ge 0$ \textbf{and} l$[$left$-5]=$ <<,>> \textbf{and} l$[$left$-1]=$ <<,>>}
		\State sng1$\gets [\, ]$
		\State r$\gets [\, ]$
				\State res1$\gets$ l$[$left$-4]$.$[$pos, singular, cow$]$
		\For{\textbf{from }$i=0$\textbf{ to} len(res1)$ - 1$}
		...
		\State \Return $[$<<он>>, <<писали>> $]$
		\EndIf
			\ElsIf{left$-6\ge 0$ \textbf{and} l$[$left$-6]=$ <<,>> \textbf{and} l$[$left$-1]=$ <<,>>}
		\State sng1$\gets [\, ]$
		\State r$\gets [\, ]$
		\State res1$\gets$ l$[$left$-5]$.$[$pos, singular, cow$]$
		\For{\textbf{from }$i=0$ \textbf{to} len(res1)$ - 1$}
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		...
		\State \Return $[$<<он>>, <<писали>> $]$
		\EndIf
		\ElsIf{left$-7\ge 0$ \textbf{and} l$[$left$-7]=$ <<,>> \textbf{and} l$[$left$-1]=$ <<,>>}
		\State sng1$\gets [\, ]$
		\State r$\gets [\, ]$
		\State res1$\gets$ l$[$left$-6]$.$[$pos, singular, cow$]$
		\For{\textbf{from }$i=0$ \textbf{to} len(res1)$ - 1$}
		...
		\State \Return $[$<<он>>, <<писали>> $]$
		\EndIf
					\ElsIf{left$-8\ge 0$ \textbf{and} l$[$left$-8]=$ <<,>> \textbf{and} l$[$left$-1]=$ <<,>>}
		\State sng1$\gets [\, ]$
		\State r$\gets [\, ]$
		\State res1$\gets$ l$[$left$-7]$.$[$pos, singular, cow$]$
		\For{\textbf{from }$i=0$ \textbf{to} len(res1)$ - 1$}
		...
		\Else
			\State \Return $[$<<он>>, <<писали>> $]$
		\EndIf
		\ElsIf{left$-9\ge 0$ \textbf{and} l$[$left$-9]=$ <<,>> \textbf{and} l$[$left$-1]=$ <<,>>}
		\State sng1$\gets [\, ]$
		\State r$\gets [\, ]$
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		...
		\EndIf
		\Else
		\If{left$-9\ge 0$ \textbf{and} l$[$left$-1]=$ <<,>> \textbf{and} end $=(-1)$}\Comment{Поиск начала перечисления}
		\State sng1$\gets [\,]$
		\State r $\gets [\, ]$ 
		...
		\State end $\gets 1$
				\ElsIf{len(sng1)$>0$ \textbf{and} <<Y>> $\in$ sng1}
		\State \Return $[$<<он>>, <<писали>> $]$
		\EndIf
			\EndIf
			\If{left$-8\ge 0$ \textbf{and} l$[$left$-1]=$ <<,>> \textbf{and} end $=(-1)$}
			\State sng1$\gets [\,]$
			\State r $\gets [\, ]$
			\State res1$\gets$ l$[$left$-8]$.$[$pos, singular, cow$]$
			\State res1$\gets$ list(set(res1))
			...
			\If{res1$[i][0]=$ part}
			\State sng1$\gets$ sng1 $+$ list(res$[i][1]$)
			\State r $\gets$ r $+$ list(l$[$left $-8\, :\, $ left$-1 ]$)
			\EndIf
			\EndFor	
		\algstore{bkbreak}
	\end{algorithmic}
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		...
		\If{res1$[i][0]=$ part}
		\State sng1$\gets$ sng1 $+$ list(res$[i][1]$)
				\State r $\gets$ r $+$ list(l$[$left $-7\, :\, $ left$-1 ]$)
		\EndIf
		\EndFor	
		\State sng1$\gets$ list(set(sng1))
		\If{(len(sng1) $=1$ \textbf{and} sng$\in$ sng1 \textbf{and} <<Y>> $\in$ r) \textbf{or} (len(sng1)$>1$ \textbf{and} (sng$=$<<N>> \textbf{or} pov~$=~1$) \textbf{and} <<Y>> $\in$ r)}
...
Нами были написаны функции two() (алгоритм \ref{alg24}), three() (алгоритм \ref{alg26}) и four() (алгоритм \ref{alg36}), которые анализируют словосочетания соответствующей длины.

\begin{algorithm}[!h]
	\caption{-- Анализ словосочетаний длины $2$}\label{alg24}
	\begin{algorithmic}[1]
	\Function{two}{lst}
	\State result $\gets [\, ]$\Comment{Результат выполнения проверки}
	\State pov $\gets (-1)$\Comment{Идентификатор наличия повелительного наклонения}
	\State sng $\gets[\, ]$ 
	\State ans1$\gets [\, ]$
	\For{\textbf{from} q $=0$ \textbf{to} len(lst)$-2$}
	\State res$_l \gets$ lst$[q]$.$[$pos, singular, cow$]$
	\State k$\gets 0$
	\State uns$\gets (-1)$
	\For{\textbf{from} $i= 0$ \textbf{to} len(res$_l$)$-1$}
	\algstore{bkbreak}
	\end{algorithmic}
\end{algorithm}

\begin{algorithm}[!h]
	\caption{-- Продолжение алгоритма \ref{alg24}}\label{alg25}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\If{res$_l[i][0]=$ <<1>> \textbf{or} res$_l[i][0]=$ <<b>>}
		\If{res$_l[i][1]=$ <<N>>}
		\State uns$\gets 1$
			\EndIf
		\EndIf
		\EndFor
		\State res$_r \gets$ lst$[q+1]$.$[$pos, singular, cow$]$
		\For{\textbf{from} $i=0$ \textbf{to} len(res$_r$)$-1$}
		\If{res$_r[i][0]=$ <<5>>}
		\State k$\gets$ k$+1$
		\ElsIf{res$_r[i][0]=$ <<1>> \textbf{or} res$_r[i][0]=$ <<b>>}
		\If{res$_r[i][1]=$ <<N>>}
		\State uns $\gets 1$ 
		\EndIf
		\EndIf
		...
		\EndIf
			\For{\textbf{from} i $=0$ \textbf{to} len(res$_l$)$-1$}
		\For{\textbf{from} j $=0$ \textbf{to} len(res$_r$)$-1$}
		\State res $\gets$ (ans).simple\_rules.(res$_l[i]$)(res$_r[j]$) \Comment{Записываем ответ для данного словосочетания из таблицы simple\_rules, в которой хранятся правила для словосочетаний длины $2$}
		\If{len(res) $>0$}
		\For{\textbf{from} $r=0$ \textbf{to} len(res)$-1$}
		\State ans$\gets$ ans $+$ res$[r][0]$
			\EndFor
		\EndIf
			\EndFor
			\EndFor
		\State ans1$\gets$ list(set(ans1))
		\If{pov$=1$ \textbf{and} uns $=(-1)$} \Comment{Если правый операнд может быть в повелительном наклонении, а слева стоит подлежащее в единственном числе}
		\If{<<N>> $\in$ ans1}
		\State result$\gets$ result $+$ $[$<<N>>$]$
		\ElsIf{<<Y>> $\in$ ans1}
		\State result $\gets$ result $+$ $[$<<Y>>$]$
		\Else
		\State result $\gets$ result $+$ $[$<<E>>$]$\Comment{<<E>>~--- незнакомое сочетание}
		\EndIf
		\Else
		\If{<<Y>> $\in$ ans1}
		\State result $\gets$ result $+$ $[$<<Y>>$]$
...
\pagebreak
\begin{algorithm}[!h]
	\caption{-- Продолжение алгоритма \ref{alg25}}\label{alg35}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
			\State result$\gets$ result $+$ $[$<<N>>$]$
				\Else
...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		...
		\Function{three}{lst}
		\State result $\gets [\, ]$ 
		\For{\textbf{from} $i=0$ \textbf{to} len(lst)$-3$}
		\State res1 $\gets$ lst$[i]$.$[$pos, singular, cow$]$
		\State res2 $\gets$ lst$[i + 1]$.$[$pos, singular, cow$]$
		\State res3 $\gets$ lst$[i + 2]$.$[$pos, singular, cow$]$
		\For{\textbf{from} $i1 =0$ \textbf{to} len(res1)$-1$}
		\For{\textbf{from} $i2 =0$ \textbf{to} len(res2)$-1$}
		\For{\textbf{from} $i3 =0$ \textbf{to} len(res3)$-1$}
		\State res $\gets$ (ans).add3\_r.(res1$[i1]$)(res1$[i2]$)(res3$[i3]$) \Comment{Записываем ответ для данного словосочетания из таблицы add3\_r, в которой хранятся правила для словосочетаний длины $3$}
		\If{len(res)$>0$}
		\For{\textbf{from} $r=0$ to len(res)$-1$}
		...
		\If{len(result)$>0$}
		\If{<<N>> $\in$ result}
		\State \Return $[$<<N>>$]$\Comment{Если на каком-то этапе сработало правило ошибки, не проверяем дальше}
		\EndIf
		\EndIf
		\EndFor
		\State \Return result
...
\newpage
\begin{algorithm}[!h]
	\caption{-- Анализ словосочетаний длины 4}\label{alg28}
	\begin{algorithmic}[1]
		%\algrestore{bkbreak}
		\Function{four}{lst}
		\State result $\gets [\, ]$ 
			\For{\textbf{from} $i=0$ \textbf{to} len(lst)$-4$}
		\State res1 $\gets$ lst$[i]$.$[$pos, singular, cow$]$
		\State res2 $\gets$ lst$[i + 1]$.$[$pos, singular, cow$]$
		\State res3 $\gets$ lst$[i + 2]$.$[$pos, singular, cow$]$
		...
		\For{\textbf{from} $i2 =0$ \textbf{to} len(res2)$-1$}
		\For{\textbf{from} $i3 =0$ \textbf{to} len(res3)$-1$}
		\For{\textbf{from} $i4 =0$ \textbf{to} len(res4)$-1$}
		\State res $\gets$ (ans).add4\_r.(res1$[i1]$)(res1$[i2]$)(res3$[i3]$)(res4$[i4]$) \Comment{Записываем ответ для данного словосочетания из таблицы add4\_r, в которой хранятся правила для словосочетаний длины $4$}
		\If{len(res)$>0$}
		\For{\textbf{from} $r=0$ to len(res)$-1$}
		\State result$\gets$ result $+$ res$[r][0]$
		\EndFor
...
Всё это запускается при помощи функции check() (см. алгоритм \ref{alg36}).

\begin{algorithm}[!h]
	\caption{~-- Описание функции check()}\label{alg36}
	\begin{algorithmic}[1]
		%\algrestore{bkbreak}
		\Function{check}{l}
		\State result $\gets [\, ]$ 
		\While{<<и>> $\in$ l \textbf{or} <<или>> $\in$ l} \Comment{Упрощение предложения посредством замены перечислений}
		\State l $\gets$ comma(l)
		\EndWhile
		\If{len(l)$>3$}
		\State result $\gets$ result + four(l)
		\EndIf
		\algstore{bkbreak}
	\end{algorithmic}
\end{algorithm}
\pagebreak
\begin{algorithm}[!h]
	\caption{~-- Продолжение алгоритма \ref{alg36}}\label{alg37}
	\begin{algorithmic}[1]
		\algrestore{bkbreak}
		\If{len(l)$>2$}
		\State result $\gets$ result $+$ three(l)
		\EndIf
		\If{len(l)$>1$}
		\State result $\gets$ result $+$ two(l)
		\EndIf
		\If{len(l)$=1$} \Comment{Одиночное слово не может быть ошибочным с точки зрения согласования единственного и множественного числа}
			\State result$\gets$ result $+$ [<<Y>>]
		\EndIf
		\State \Return result
		\EndFunction
...
Анализирует полученные результаты функция res() (алгоритм \ref{alg38}). 

\begin{algorithm}[!h]
	\caption{~-- Описание функции res()}\label{alg38}
	\begin{algorithmic}[1]
	%	\algrestore{bkbreak}
		\Function{res}{r}
		\If{<<N>> $\in $ r}
			\State print(<<Ошибка в согласовании единственного и множественного числа>>)
		\ElsIf{<<Y>> $\in $ r}
		\State print(<<Ошибок в согласовании единственного и множественного числа не обнаружено>>)
		\Else
		\State print(<<Есть незнакомые сочетания>>)
		\EndIf
		\EndFunction
		%\algstore{bkbreak}
	\end{algorithmic}
...
Рассмотрим предложение \textit{<<Он хотел организовать проверку знаний требований охраны труда, купить себе пони и выспаться>>}. Запишем его в строку str. Чтобы проверить согласование единственного и множественного числа, необходимо вызвать res(check(space(str))).

Фунция spaсe() преобразует str в список $l$ = [\textit{<<он>>}, \textit{<<хотел>>}, \textit{<<организовать>>}, \textit{<<проверку>>}, \textit{<<знаний>>}, \textit{<<требований>>}, \textit{<<охраны>>}, \textit{<<труда>>}, \textit{<<,>>}, \textit{<<купить>>}, \textit{<<себе>>}, \textit{<<пони>>}, \textit{<<и>>}, \textit{<<выспаться>>}].

Далее, обрабатывая список $l$, фунция comma() определит id1 $=12$, left $=2$, right $=13$. Проверит, что одиночный инфинитив \textit{<<выспаться>>} не является ошибочным, как и словосочетания \textit{<<купить себе пони>>} (правило 16 таблицы \ref{tab2}) и \textit{<<организовать проверку знаний требований охраны труда>>} (правила 5, 69, 78, 79, 65 таблицы~\ref{tab1}).

После этого функцией comma() список будет преобразован в [\textit{<<он>>}, \textit{<<хотел>>}, \textit{<<учить>>}], который будет проверен функцией check(). В результате срабатывания правил 4, 6 таблицы \ref{tab1} будет получен список result = [\textit{<<Y>>}, \textit{<<Y>>}].
